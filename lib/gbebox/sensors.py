...
sensor_libs = load_libraries([
    ...
    "drivers.ds3231", "drivers.seesaw", "drivers.stemma_soil_sensor",
    "drivers.fan_rpm", "drivers.sht35"
])
...
class SensorManager:
    ...
    def _initialize_sensors(self):
        ...
        self._sensors['ina'] = self._initialize_sensor('drivers.ina219_gbe', 'INA219', self.i2c0)

        # Environmental sensors (all on I2C1 - optional accessories)
        self._sensors['veml'] = self._initialize_sensor(
            'drivers.veml7700',
            'VEML7700',
            address=0x10,
            i2c=self.i2c1,
        ...
        self._sensors['mpl'] = self._initialize_sensor(
            'drivers.mpl3115a2',
            'MPL3115A2',
            self.i2c1,
            mode=sensor_libs['drivers.mpl3115a2'].MPL3115A2.PRESSURE
            if sensor_libs['drivers.mpl3115a2'] else None
        )

        self._sensors['seesaw'] = self._initialize_sensor(
            'drivers.stemma_soil_sensor',
            'StemmaSoilSensor',
            self.i2c1
        )

        # High-precision temperature and humidity sensor (SHT35)
        self._sensors['sht35'] = self._initialize_sensor(
            'drivers.sht35',
            'SHT35',
            self.i2c1,
            address=0x44
        ...
        if sensor_libs['drivers.fan_rpm']:
            self._sensors['rpm'] = sensor_libs['drivers.fan_rpm'].FanRPM(
                pin_num=board["pins"]["rpm"]
            )
        else:
            self._sensors['rpm'] = None

        # CO2/Temperature/Humidity sensor
        self._sensors['scd'] = self._initialize_sensor('drivers.scd4x', 'SCD4X', self.i2c1)
        if self._sensors['scd']:
            self._sensors['scd'].temperature_offset = 0
            if self._sensors['mpl']:
                self._sensors['scd'].set_ambient_pressure(
                    round(self._sensors['mpl'].pressure() / 100)
                )
            self._sensors['scd'].start_periodic_measurement()

    async def reinitialize_scd4x_if_needed(self):
        """Reinitialize the SCD4x sensor if it is not responding."""
    ...
    def temperature(self):
        ...
        if self._sensors['sht35']:
            return SensorReading(
                lambda: round(self._sensors['sht35'].temperature, 1)
                if self._sensors['sht35'].temperature is not None
                else None,
                "C",
                "sht35",
            )
            ...
            return SensorReading(
                lambda: round(self._sensors['mpl'].temperature(), 1)
                if self._sensors['mpl'].temperature() is not None
                else None,
                "C",
                "mpl3115a2",
            )
            ...
            return SensorReading(
                lambda: round(self._sensors['scd'].temperature, 1)
                if self._sensors['scd'].temperature is not None
                else None,
                "C",
                "scd4x",
            )
            ...
            return SensorReading(
                lambda: round(self._sensors['seesaw'].get_temp(), 1)
                if self._sensors['seesaw'].get_temp() is not None
                else None,
                "C",
                "seesaw",
            )
    ...
    def humidity(self):
        ...
        if self._sensors['sht35']:
            return SensorReading(
                lambda: round(self._sensors['sht35'].humidity, 1)
                if self._sensors['sht35'].humidity is not None
                else None,
                "%",
                "sht35",
            )
            ...
            return SensorReading(
                lambda: round(self._sensors['scd'].relative_humidity)
                if self._sensors['scd'].relative_humidity is not None
                else None,
                "%",
                "scd4x",
            )
    ...
    def co2(self):
        ...
        if self._sensors['scd']:
            return SensorReading(
                lambda: self._sensors['scd'].CO2
                if self._sensors['scd'].CO2 is not None
                else None,
                "ppm",
                "scd4x",
            )
    ...
    def pressure(self):
        ...
        if self._sensors['mpl']:
            return SensorReading(
                lambda: round(self._sensors['mpl'].pressure())
                if self._sensors['mpl'].pressure() is not None
                else None,
                "Pa",
                "mpl3115a2",
            )
    ...
    def lux(self):
        ...
        if self._sensors['veml']:
            return SensorReading(
                lambda: self._sensors['veml'].read_lux()
                if self._sensors['veml'].read_lux() is not None
                else None,
                "lx",
                "veml7700",
            )
    ...
    def voltage(self):
        ...
        if self._sensors['ina']:
            return SensorReading(
                lambda: round(self._sensors['ina'].bus_voltage, 1)
                if self._sensors['ina'].bus_voltage is not None
                else None,
                "V",
                "ina219",
            )
    ...
    def current(self):
        ...
        if self._sensors['ina']:
            return SensorReading(
                lambda: round(self._sensors['ina'].current)
                if self._sensors['ina'].current is not None
                else None,
                "mA",
                "ina219",
            )
    ...
    def power(self):
        ...
        if self._sensors['ina']:
            return SensorReading(
                lambda: round(self._sensors['ina'].power, 1)
                if self._sensors['ina'].power is not None
                else None,
                "W",
                "ina219",
            )
    ...
    def fan_speed(self):
        ...
        if self._sensors['rpm']:
            return SensorReading(
                lambda: self._sensors['rpm'].get_rpm()
                if self._sensors['rpm'].get_rpm() is not None
                else None,
                "rpm",
                "fan_rpm",
            )
    ...
    def moisture(self):
        ...
        if self._sensors['seesaw']:
            return SensorReading(
                lambda: self._sensors['seesaw'].get_moisture()
                if self._sensors['seesaw'].get_moisture() is not None
                else None,
                None,
                "seesaw",
            )
    ...
    def scan_i2c1_bus(self, confirm_n=3, gap_ms=5):
        """Passively scan I2C1 and return only addresses that appear in all confirm_n scans.
           Uses I2C.scan() (non-intrusive) instead of writing to every address."""
        try:
            scans = []
            for _ in range(max(1, confirm_n)):
                scans.append(set(self.i2c1.scan()))   # passive probe
                if confirm_n > 1:
            ...
            if not scans:
                ...
                # Iterative intersection for MicroPython compatibility
                stable = scans[0]
                for s in scans[1:]:
                    stable = stable & s

            stable_list = sorted(stable)
            self._last_i2c1_scan = stable_list
            return stable_list

        except Exception as e:
            print(f"I2C1 scan error: {e}")
            return sorted(self._last_i2c1_scan or [])
    ...
    def reload_i2c1_sensors(self):
        ...
        # Store I2C0 sensors (power monitoring and fan RPM)
        ...
        self._sensors['scd'] = self._initialize_sensor(
            'drivers.scd4x',
            'SCD4X',
            self.i2c1
        )
    ...
    async def monitor_sensor_changes(self, scan_interval=300):
